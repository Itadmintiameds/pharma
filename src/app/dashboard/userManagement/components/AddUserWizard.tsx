...
export default function AddUserWizard({ onBack }: AddUserWizardProps) {
      ...
      <div className="flex flex-col gap-3 w-full">
        ...
        <div className="flex justify-between items-center w-full min-h-[50px] relative px-4">
          ...
          <div className="flex flex-col items-center bg-gray-50 px-4">
            ...
            <span
              className={`text-[12px] ${step >= 1 ? "text-[#7D32FC] font-semibold" : "text-gray-500 font-medium"}`}
            >
              Personal Info
            </span>
          </div>
          ...
          <div className="flex flex-col items-center bg-gray-50 px-4">
            ...
            <span
              className={`text-[12px] ${step >= 2 ? "text-[#7D32FC] font-semibold" : "text-gray-500 font-medium"}`}
            >
              Role Assignment
            </span>
          </div>
          ...
          <div className="flex flex-col items-center bg-gray-50 px-1">
            ...
            <span
              className={`text-[12px] ${step >= 3 ? "text-[#7D32FC] font-semibold" : "text-gray-500 font-medium"}`}
            >
              Complete
            </span>
          </div>
  ...
  const renderStep1 = () => (
    <div className="w-full flex-1 p-[14px] gap-[24px] rounded-[12px] border-[0.89px] border-pneutral-100 bg-white flex flex-col shadow-sm">
      <h3 className="text-lg font-semibold text-gray-900 shrink-">
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-3">
        ...
        <div className="w-full">
          ...
          <div className="flex h-12 w-full items-center justify-between rounded-md border border-dashed border-pneutral-300 bg-gray-50 transition-all px-3 cursor-pointer hover:bg-gray-100">
            <span className="text-p4 text-pneutral-500">
      ...
      <div className="w-full flex-1 flex gap-[10px] items-stretch min-h-0">
        ...
        <div className="w-[210px] h-full p-[16px] flex flex-col gap-[8px] rounded-[12px] border-[1px] border-gray-200 bg-white shadow-sm shrink-0">
          <div className="w-full h-[40px] p-[8px] border-b-[1px] border-gray-200 shrink-0">
            <h3 className="font-medium text-[16px] leading-[24px] text-[#3C3D3A]">
              Modules
            </h3>
          </div>
          ...
          <div className="w-full flex-1 overflow-auto flex min-h-0">
            ...
            <div className="w-[180px] flex flex-col shrink-0">
              <div className="w-[180px] h-[72px] p-[16px_8px_16px_8px] border-b-[1px] border-[#EAEAE9] bg-[#F9F9F8] shrink-0 flex items-center gap-[10px]">
                <span className="h-[20px] text-[14px] leading-[20px] font-semibold text-[#1E1E1D]">
                  Permission
                </span>
              </div>
              {permissions.map((p, i) => (
                <div
                  key={i}
                  className="w-[180px] h-[68px] p-[12px_8px_12px_8px] border-b-[1px] border-gray-200 shrink-0 flex items-center gap-[4px]"
                >
                  <span className="h-[20px] text-[14px] leading-[20px] font-normal text-[#1E1E1D] truncate">
                    {p}
                  </span>
                </div>
              ))}
            ...
            {cols.map((c) => (
              <div key={c} className="flex-1 flex flex-col min-w-[78px]">
                ...
                {permissions.map((p, i) => (
                  <div
                    key={i}
                    className="w-full h-[68px] p-[12px_8px_12px_8px] border-b-[1px] border-gray-200 shrink-0 flex items-center justify-center gap-[4px]"
                  >
                    <input
                      type="checkbox"
  ...
  return (
    <div className="flex flex-col gap-3 w-full justify-between items-stretch">
      <div className="flex flex-col gap-2 w-full">{renderHeader()}</div>
